/* OAuth 2.0 / OpenID Connect for Google. No browser-supplied
...
const enabled = {
  ...
  apple: false, // Compatibility for older clients; Apple login has been removed.
};
...
function start(provider, linkUid = null) {
  if (provider !== 'google' || !enabled.google) throw new Error('auth-not-configured');
...
async function verifyIdToken(provider, token, nonce) {
  try {
    ...
    return { sub: claims.sub, email: String(claims.email).trim().toLowerCase(),
      name: String(claims.name || '').trim().slice(0, 80) };
  } catch (error) {
