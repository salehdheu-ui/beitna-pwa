...
/* أول رسمة في الجلسة يجب أن تظهر ساكنة: قياسًا على جهاز أبطأ بستّ مرات
   كان السبلاش ينكشف عند شفافية 0.22 والمحتوى ما زال يزحف من
   translateY(6px) — فتُرى الصفحة وهي «تتحرك» أول ما تُفتح. */
const enterToggle = (app.match(/classList\.toggle\('enter'[^)]*\)/) || [''])[0];
if (enterToggle.includes('!firstPaint')) ok('أول رسمة بلا حركة دخول');
else bad('أول رسمة تشغّل حركة الدخول — الصفحة تتحرك عند الفتح');

/* وشريط التثبيت لا يزاحم لحظة الانكشاف */
...
/* سبب قفزة الجرس لم يكن الإيموجي بل صندوقه: أبعاد حرة + transform عند
   التفعيل. نحرس السبب — صندوق ثابت 28×28 — لا الحلّ القديم (رسمة SVG
   شذّت لونًا عن بقية الأيقونات). القياس: الصندوق لم يتزحزح بكسلًا واحدًا
   بين الحالتين بعد تثبيته. */
if (/\.navitem\s+\.ic\s*\{[^}]*width:\s*28px[^}]*height:\s*28px/s.test(css)) {
  ok('صندوق أيقونات الشريط ثابت الأبعاد، فلا يقفز الإيموجي');
} else bad('صندوق أيقونة الشريط بلا أبعاد ثابتة — الإيموجي سيقفز بين الخطوط');

/* والشريط الجانبي عمودي: ارتفاع مفروض على العنصر يجعله بطول العمود كله */
